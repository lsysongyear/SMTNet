"""
Plot macro_acc boxplot + scatter across all models (except vlaai) for MEG NIPS.
Each seed is one data point — shows cross-seed stability per model.
"""
...
# --- Collect data ---
model_data = {}  # model_name -> [macro_acc per seed, ...]
...
for model_dir in sorted(os.listdir(os.path.join(RESULTS_DIR, "speech-detection"))):
    ...
    model_path = os.path.join(RESULTS_DIR, "speech-detection", model_dir)

    # Find the params directory (no split: level in MEG NIPS)
    param_dirs = [d for d in os.listdir(model_path) if os.path.isdir(os.path.join(model_path, d))]
    if len(param_dirs) == 0:
        continue
    param_dir = os.path.join(model_path, param_dirs[0])

    accs = []
    for run_dir in sorted(os.listdir(param_dir)):
        if not run_dir.startswith("run"):
            continue
        log_path = os.path.join(param_dir, run_dir, "test_log.json")
        if not os.path.exists(log_path):
            continue
        with open(log_path) as f:
            data = json.load(f)
        # Extract macro_acc from the first metric key (top1:...)
        top1_key = [k for k in data.keys() if k.startswith("top1:")][0]
        accs.append(data[top1_key]["test_macro_acc"])

    if accs:
        model_data[model_name] = accs
        print(f"{model_name}: n={len(accs)}, mean={np.mean(accs):.4f}, std={np.std(accs):.4f}")

# --- Reorder: baselines first, BrainMagic ablations, v7 on the right ---
...
for i, d in enumerate(data):
    ...
    ax.scatter(np.full_like(d, i + 1) + jitter, d, alpha=0.6, s=30,
...
ax.set_title("Cross-Seed Macro Accuracy by Model (MEG NIPS, per_sub_5t5v, 5 seeds)")
