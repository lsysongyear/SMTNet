...
class Brain_Magic_speech_v1(nn.Module):
    def __init__(self, input_dim=306, attention_dim=128, output_dim=1,
        ...
        self.cross_attn1 = nn.MultiheadAttention(
            embed_dim=attention_dim, num_heads=4, batch_first=True)
    ...
    def forward(self, x, subject_ids=None, day_ids=None):
        ...
        if subject_ids is not None and day_ids is not None:
            indices = subject_ids * self.n_days + day_ids   # [B] 0..74
            attn_outputs = []
            for i in range(B):
                idx = indices[i].item()
            ...
            # 无 subject/day 信息时使用第 0 层（兼容旧代码）
        ...
        # 3. 交叉注意力融合（替代简单拼接）
        # 逐时间步独立融合：reshape [B, C, T] → [B*T, 1, C] → cross-attn → [B*T, C] → [B, C, T]
        B, C, T = short_feat.shape
        s = short_feat.transpose(1, 2).reshape(B * T, C).unsqueeze(1)   # [B*T, 1, C]
        e = encoded_feat.transpose(1, 2).reshape(B * T, C).unsqueeze(1) # [B*T, 1, C]

        # 双向交叉注意力
        s2e, _ = self.cross_attn1(s, e, e)   # short_feat 关注 encoded_feat
        e2s, _ = self.cross_attn2(e, s, s)   # encoded_feat 关注 short_feat

        fused = torch.cat([s2e.squeeze(1), e2s.squeeze(1)], dim=-1)  # [B*T, 2*C]
        fused = self.fusion_proj(fused)                               # [B*T, C]
        fused = fused.reshape(B, T, C).transpose(1, 2)               # [B, C, T]
...
if __name__ == "__main__":
    ...
    day = torch.tensor([0, 1, 2, 0])    # day 1, 2, 3, 1
    y = model(x, subject_ids=subj, day_ids=day)
    print(y.shape)  # [4, 3000]
    # Verify different subjects use different attention
    y2 = model(x, subject_ids=torch.tensor([0,1,0,0]), day_ids=torch.tensor([0,0,0,0]))
    print(f"Subject/day routing works: {(y[0] - y2[0]).abs().sum():.6f} (subj00==subj00), {(y[0] - y2[1]).abs().sum():.6f} (subj00!=subj01)")
