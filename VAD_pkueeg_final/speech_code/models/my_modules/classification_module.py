...
from speech_code.models.my_modules.BrainNetwork_v2 import Brain_Magic_speech_v2
from speech_code.models.my_modules.BrainNetwork_v4 import Brain_Magic_speech_v1 as Brain_Magic_speech_v4
from speech_code.models.my_modules.BrainNetwork_v5 import Brain_Magic_speech_v1 as Brain_Magic_speech_v5
from speech_code.models.my_modules.BrainNetwork_v6 import Brain_Magic_speech_v1 as Brain_Magic_speech_v6
...
class ClassificationModule(LightningModule):
    ...
    """
    def __init__(self, model_config: list[tuple[str, dict]], n_classes: int, optimizer_config: dict, loss_config: dict, margin_weight: list, sfreq: float = 250.0):
        """
        Args:
            ...
            sfreq: EEG 输入采样率（Hz），用于计算上采样比例使输出对齐 250Hz 标签
        """
        super().__init__()
        # 保存超参数到 checkpoint（用于后续加载和复现）
        self.save_hyperparameters()

        # 标签始终为 250Hz，上采样比例 = 250 / sfreq
        self.scale_factor = 250.0 / sfreq
    ...
    def forward(self, x, subject_ids=None, day_ids=None):
        ...
        for module in self.modules_list:
            if isinstance(module, (Brain_Magic_speech_v4, Brain_Magic_speech_v5, Brain_Magic_speech_v6, Brain_Magic_speech_v7,
                                   CNNLSTM, DilatedConv, AWaveNet, VLAAI, EEGConformer,
                                   BrainMagic_NoSubjectAttn, BrainMagic_NoShortConv, BrainMagic_NoFeatureEncoder)) or getattr(module, "requires_subject_ids", False):
                x = module(x, subject_ids=subject_ids)
            elif isinstance(module, (Brain_Magic_speech_v1, Brain_Magic_speech_v2)):
                x = module(x, subject_ids=subject_ids, day_ids=day_ids)
            else:
                x = module(x)
        ...
        # 使用 size 参数确保输出长度与标签精确对齐
        target_len = int(input_len * self.scale_factor)
        x = F.interpolate(
            x,
            size=target_len,
            mode='linear',
        ...
        x = x.squeeze(1)                               # [batch, 1, target_len] → [batch, target_len]
    ...
    def _extract_ids(self, batch):
        ...
        if isinstance(info, dict) and 'subject' in info:
            # default_collate 将 list of dicts 合并为单个 dict, 每个值是 list
            subjects = info['subject']
            days = info.get('day_id', [0] * len(subjects))
            if isinstance(subjects[0], str):
                subj_ids = torch.tensor([int(s) - 1 for s in subjects], device=self.device)
            else:
                subj_ids = torch.tensor([s - 1 for s in subjects], device=self.device)
        ...
        if isinstance(info, (list, tuple)) and len(info) > 0 and isinstance(info[0], dict):
            if 'subject' not in info[0]:
                return None, None
            subject_ids = torch.tensor([int(d['subject']) - 1 for d in info], device=self.device)
