...
class EEGConformer(nn.Module):
    def __init__(self,
        ...
        self.spatial_attentions = nn.ModuleList([
            self._build_spatial_attention(input_dim)
    ...
    def forward(self, x, subject_ids=None):
        ...
        # Subject-specific spatial attention
        x_t = x.transpose(1, 2)                   # (B, T, input_dim)
        if subject_ids is not None:
            outs = []
            for i in range(B):
                idx = subject_ids[i].item()
                outs.append(self.spatial_attentions[idx](x_t[i:i+1]))
            x_t = torch.cat(outs, dim=0)
        else:
            x_t = self.spatial_attentions[0](x_t)
