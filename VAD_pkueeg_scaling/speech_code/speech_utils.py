...
def _extract_ids_from_batch(batch, device):
    """从 batch 中提取 subject_ids, day_ids 张量。batch[2] 是 info 的 list 或 dict。"""
    if len(batch) < 3:
        return None, None
    info = batch[2]
    # PyTorch default_collate 将 dict list 合并为 {key: [values]} 或 {key: tensor}
    if isinstance(info, dict):
        if 'subject' not in info:
            return None, None
        subjects = info['subject']
        days = info.get('day_id', None)
        if subjects is None:
            return None, None
        # subjects may be list of strings or tensor
        if isinstance(subjects, torch.Tensor):
            subj_ids = subjects.to(device)
        else:
            subj_ids = torch.tensor([int(s) - 1 for s in subjects], device=device)
        if days is not None:
            if isinstance(days, torch.Tensor):
                day_ids = days.to(device)
            else:
                day_ids = torch.tensor([int(d) for d in days], device=device)
        else:
            day_ids = torch.zeros(len(subj_ids), dtype=torch.long, device=device)
        return subj_ids, day_ids
    elif isinstance(info, (list, tuple)) and len(info) > 0 and isinstance(info[0], dict):
        if 'subject' not in info[0]:
            return None, None
        subjects = [int(d['subject']) - 1 for d in info]
        days = [d.get('day_id', 0) for d in info]
        return torch.tensor(subjects, device=device), torch.tensor(days, device=device)
    return None, None
    ...
    for ckpt in tqdm(ckpt_paths, desc="Validating models"):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                subj, day = _extract_ids_from_batch(batch, module.device)
                probs = module(x, subject_ids=subj, day_ids=day)
...
def my_run_test(test_loader, module_path, labels, weight, threshold=[0.5, 0.6, 0.7, 0.8, 0.9], prefix="test_", test_results_path = None, fixed_ckpt_path = None, fixed_threshold = None):
    ...
    for ckpt in tqdm(ckpt_paths, desc="Testing models"):
        ...
        for t in threshold:
            ...
            f1 = f1_score(all_targets_np, y_pred, average='macro')
            if f1 > best_f1:
                best_f1 = f1
                best_ckpt = ckpt
                best_threshold = t
