...
def collect_results(results_root):
    ...
    for scale_dir in sorted(results_root.glob("scale_*")):
        ...
        row = {
            ...
            "train_subject_story_pair_count": manifest["train_subject_story_pair_count"],
            ...
            "train_window_hours_with_overlap": manifest["train_window_hours_with_overlap"],
...
def _find_original_baseline():
    old_root = PROJECT_ROOT.parent / "VAD_SEM4Lang" / "results"
    candidates = [
        path
        for path in old_root.rglob("pretrain_eval_summary.json")
        if "model:brain_magic_speech_v7-dataset:sem4lang_meg" in str(path)
        and "run5:seed1_fold0" in str(path)
    ]
...
def write_outputs(rows, subject_values, output_dir):
    ...
    ax.plot(x, y, marker="o", linewidth=2, label="Mean across 12 subjects")
    ...
    ax.set_title("SEM4Lang Training-Duration Scaling")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("SEM4Lang Per-Subject Scaling Results")
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(output_dir / "scaling_boxplot_macro_acc.png", dpi=180)
    plt.close(fig)

    full_rows = [row for row in rows if row["scale_percent"] == 100]
    baseline_path = _find_original_baseline()
    if full_rows and baseline_path is not None:
        with baseline_path.open(encoding="utf-8") as handle:
            baseline = json.load(handle)
        comparison = {
            "baseline_summary": str(baseline_path.resolve()),
            "baseline_macro_acc": baseline["macro_acc"],
            "scaling_100_macro_acc": {
                "mean": full_rows[0]["macro_acc_mean"],
                "std": full_rows[0]["macro_acc_subject_std"],
            },
            "mean_difference": full_rows[0]["macro_acc_mean"]
            - baseline["macro_acc"]["mean"],
        }
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="results/scaling/summary_duration_prefix_train_only_seed5_subset1001_model1"
    )
