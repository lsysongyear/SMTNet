...
# ============================================================================
# SEM4Lang_MEG - SEM4Lang MEG VAD Dataset
# ============================================================================
class SEM4Lang_MEG(Dataset):
    """
    SEM4Lang MEG frame-level VAD Dataset.

    MEG: (204, T) at 64Hz, 204 gradiometer channels, LP-30 filtered.
    Labels: Silero VAD TSV at 250Hz (voice/silence).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        include_subjects = _normalize_sem4lang_subjects(include_subjects)
        exclude_subjects = set(_normalize_sem4lang_subjects(exclude_subjects) or [])
        # stories from split are int, parsed stories are str — normalize to str
        requested_stories = set(str(s) for s in stories) if stories is not None else None
        ...
        for filename in sorted(os.listdir(data_path)):
            if not filename.endswith(".npy"):
                continue
            parsed = self._parse_meg_name(filename)
            if parsed is None:
                continue
            ...
            if requested_stories is not None and story not in requested_stories:
                continue
            meg_files.append((subject, story, os.path.join(data_path, filename)))

        if len(meg_files) == 0:
            raise ValueError(
                "No SEM4Lang MEG files found. Check data_path, subjects, stories."
            )

        files_by_subject = {}
        for subject, story, meg_path in meg_files:
            files_by_subject.setdefault(subject, []).append((story, meg_path))

        for subject in sorted(files_by_subject):
            subject_items = []
            for story, meg_path in sorted(
                files_by_subject[subject], key=lambda item: int(item[0])
            ):
                arr = np.load(meg_path, mmap_mode='r')
                full_samples = int(arr.shape[1])
                meg_duration = full_samples / self.sfreq
                labels = self._get_labels_for_story(story, meg_duration)
                subject_items.append((story, meg_path, labels, full_samples))

            target, allocations = prefix_allocation(
                [item[3] for item in subject_items], self.recording_fraction
            )
            unit_records = []
            for (story, meg_path, labels, full_samples), keep_samples in zip(
                ...
                keep_labels = int(keep_samples * self.label_sfreq / self.sfreq)
                if keep_labels > 0:
                    self._collect_samples(
                        subject, story, meg_path, labels[:keep_labels]
                    )
        ...
        if len(self.samples) == 0:
            raise ValueError("No samples found for SEM4Lang. Check window length and labels.")

    @staticmethod
    def _parse_meg_name(filename):
        stem = filename[:-4] if filename.endswith(".npy") else filename
        parts = stem.split("_")
        if len(parts) < 4:
            return None
        subject = parts[0]  # sub-XX
        audio_key = parts[1]  # story-YY
        preproc = parts[2]  # LP-30
        fs_tag = parts[3]  # 64Hz
        if not subject.startswith("sub-") or not audio_key.startswith("story-"):
            return None
        story = str(int(audio_key.replace("story-", "", 1)))
        return subject, story, preproc, fs_tag

    def _get_labels_for_story(self, story, meg_duration):
        target_samples = int(meg_duration * self.label_sfreq)
        cache_key = (story, target_samples)
        if cache_key in self.label_cache:
            return self.label_cache[cache_key]

        tsv_path = os.path.join(self.label_path, f"segments_{int(story):02d}.tsv")
        if not os.path.exists(tsv_path):
    ...
    def _collect_samples(self, subject, story, meg_path, speech_labels, stride=None):
        time_window_samples = self.points_per_sample_label
        if stride is None:
        ...
        for i in range(0, len(speech_labels), stride):
            ...
            if self.data_type == 'train':
                ...
                sample_labels = speech_labels[jittered_i:jittered_i + time_window_samples]
                onset_i = jittered_i
            self.samples.append((subject, story, meg_path, onset_i / self.label_sfreq, sample_labels))

    def _load_meg(self, meg_path):
        if meg_path not in self.open_files:
            self.open_files[meg_path] = np.load(meg_path, mmap_mode='r')
        return self.open_files[meg_path]

    def __getitem__(self, idx):
        if idx >= len(self.samples):
            raise IndexError(f"Index {idx} out of bounds for dataset of size {len(self.samples)}")
        subject, story, meg_path, onset, label = self.samples[idx]
        meg_data = self._load_meg(meg_path)  # (204, T)

        # Downsample labels from label_sfreq to sfreq (e.g. 250->100 Hz)
        if self.label_sfreq != self.sfreq:
            label_len = len(label)
            target_len = int(label_len * self.sfreq / self.label_sfreq)
            idx_arr = (np.arange(target_len + 1) * label_len / target_len).astype(int)
            label = np.array([label[idx_arr[i]:idx_arr[i+1]].max() for i in range(target_len)], dtype=label.dtype)

        delay_samples = self.delay
        start = max(0, int(onset * self.sfreq) + delay_samples)
        end = start + self.points_per_sample
        data = np.asarray(meg_data[:, start:end], dtype=np.float32)
...
def _normalize_sem4lang_subjects(subjects):
    if subjects is None:
        return None
    normalized = []
    for s in subjects:
        ...
        if s.startswith("sub-"):
            ...
            normalized.append(f"sub-{int(s):02d}")
    return normalized
...
DATASETS = {
    ...
    "eeg_speech_v1": EEG_Speech_v1,  # PKU EEG speech detection dataset
    "sem4lang_meg": SEM4Lang_MEG,     # SEM4Lang MEG VAD dataset
}
