...
def test_only(run_dir):
    ...
    if split_strategy == 'whole_data' and 'split_pairs' in config.get('data', {}):
        base_cfg = config["data"]["datasets"]["test"][0][dataset_key]
...
def train(config, test_mode=False):
    ...
    if test_mode:
        ...
        # SEM4Lang test mode: use subset for quick testing
    ...
    # 获取划分（SEM4Lang: 6折=10 test, val_ratio=0.1=5 val from 50 remaining）
    n_folds = 6 if dataset_key == "sem4lang_meg" else 5
    val_ratio = 0.10 if dataset_key == "sem4lang_meg" else 0.15
    splitter = DataSplitter(all_subjects, all_stories, n_folds=n_folds,
                            seed=split_seed, val_ratio=val_ratio)
    ...
    if split_strategy == 'whole_data':
        base_cfg = config["data"]["datasets"]["train"][0][dataset_key]
