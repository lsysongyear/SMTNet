...
for value in "$SPLIT_SEED" "$SUBSET_SEED" "$MODEL_SEED"; do
    if [[ ! "$value" =~ ^[0-9]+$ ]]; then
        echo "Seeds must be non-negative integers: $value" >&2
        exit 2
    fi
done
...
echo "SEM4Lang scaling experiment"
