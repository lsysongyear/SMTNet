...
class CNN_baseline(nn.Module):
    def __init__(self):
        ...
        self.layernorm = nn.LayerNorm(57)
        self.conv1 = nn.Conv1d(in_channels=57, out_channels=256, kernel_size=25, padding=12, stride=2)
        ...
        self.conv_layer = nn.Conv2d(in_channels=1, out_channels=100, kernel_size=(17,57), padding=(8, 0))
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        x = x.transpose(1, 2)          # [batch, ch, time] → [batch, time, ch]
        x = self.layernorm(x)          # LayerNorm over channel dim (57)
        x = x.transpose(1, 2)          # [batch, time, ch] → [batch, ch, time]
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim=57, **kwargs):
        super(TransformerEncoder, self).__init__()
        self.layernorm1 = nn.LayerNorm(input_dim)
        self.conv_layer1 = nn.Conv1d(in_channels=input_dim, out_channels=32, kernel_size=3, padding='same')
        self.layernorm2 = nn.LayerNorm(32)
        self.conv_layer2 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=3, padding='same')
        self.dropout = nn.Dropout(0.5)
        self.relu = nn.ReLU()
        self.pe = PositionalEncoding(d_model=32, max_len=5000)
    ...
    def forward(self, x):
        # x: [batch, input_dim, seq_len] channel-first
        x = x.transpose(1, 2)            # [batch, seq_len, input_dim]
        x = self.layernorm1(x)           # LayerNorm(input_dim) on last dim
        ...
        relu_out = relu_out.transpose(1, 2)      # [batch, seq_len, 32]
        pe_out = self.pe(relu_out)              # [batch, seq_len, 64]
...
class ConcatCovNet(nn.Module):
    def __init__(self, input_dim=57, **kwargs):
        ...
        self.conv_pre = nn.Conv1d(in_channels=input_dim, out_channels=32, kernel_size=3, padding='same')
        self.pe = PositionalEncoding(d_model=32, max_len=5000)
    ...
    def forward(self, x):
        # x: [batch, input_dim, seq_len] channel-first
        x = x.transpose(1, 2)            # [batch, seq_len, input_dim]
        x = self.layernorm(x)            # LayerNorm(input_dim) on last dim
        x = x.transpose(1, 2)            # [batch, input_dim, seq_len]
        x = self.conv_pre(x)             # Conv1d(input_dim, 32) on [batch, input_dim, seq_len]
        x = self.relu(x)
        x = self.dropout(x)
        x = x.transpose(1, 2)            # [batch, seq_len, 32]
        x_pe = self.pe(x)               # [batch, seq_len, 64]
        x_pe = x_pe.transpose(1, 2)      # [batch, 64, seq_len]
        x = self.conv_block1(x_pe)       # [batch, 64, seq_len]
        x_in = torch.cat([x, x_pe], dim=1)  # [batch, 128, seq_len]
        x = self.conv_block2(x_in)       # [batch, 128, seq_len]
        x_in = torch.cat([x, x_in], dim=1)  # [batch, 256, seq_len]
        x = self.conv_block3(x_in)       # [batch, 256, seq_len]
