...
def _extract_ids_from_batch(batch, device):
    """从 batch 中提取 subject_ids, day_ids 张量。"""
    if len(batch) < 3:
        return None, None
    info = batch[2]

    def _parse_subj(s):
        """Parse subject id from string like 'sub-01' or '01'."""
    ...
    if isinstance(info, dict):
        if 'subject_id' in info:
            ...
            subj_ids = sids.to(device) if isinstance(sids, torch.Tensor) else torch.tensor(sids, device=device)
        elif 'subject' in info:
            subjects = info['subject']
            subj_ids = torch.tensor([_parse_subj(s) - 1 for s in subjects], device=device)
        else:
            return None, None
        days = info.get('day_id', [0] * len(subj_ids))
        day_ids = days.to(device) if isinstance(days, torch.Tensor) else torch.tensor(days, device=device)
        return subj_ids, day_ids
    elif isinstance(info, (list, tuple)) and len(info) > 0 and isinstance(info[0], dict):
        d0 = info[0]
        if 'subject_id' in d0:
            subject_ids = torch.tensor([d['subject_id'] for d in info], device=device)
        elif 'subject' in d0:
            subject_ids = torch.tensor([_parse_subj(d['subject']) - 1 for d in info], device=device)
        else:
            return None, None
        day_ids = torch.tensor([d.get('day_id', 0) for d in info], device=device)
