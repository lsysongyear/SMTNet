...
class DataSplitter:
    ...
    def __init__(self, subjects, stories, n_folds=5, seed=42, val_ratio=0.15,
        ...
        self.subject_stories_map = subject_stories_map  # {subject: [stories]}
    ...
    def get_split(self, strategy, fold):
        ...
        if strategy == 'cross_subject':
            ...
            return self._per_sub_split(fold)
    ...
    # ------------------------------------------------------------------
    # 策略 8 — 全局故事隔离划分（所有被试共享验证/测试 story）
    # ------------------------------------------------------------------
    def _per_sub_split(self, fold):
        """
        从所有被试共同拥有的 story 中选取 1 个测试和 1 个验证 story。

        所有被试共享这两个 held-out story；每名被试的其余实际 story
        全部用于训练。这样不仅同一 (subject, story) pair 不重叠，而且
        同一语音材料也不会跨被试出现在不同 partition 中。

        种子推导: shared_seed = base_seed + fold * 1000
        """
        pools = {}
        for subject in self.subjects:
            pool = (self.subject_stories_map.get(subject) if self.subject_stories_map
                    else self.stories)
            if pool is None or len(pool) < 3:
                raise ValueError(
                    f"Subject {subject} has only {len(pool) if pool else 0} stories, "
                    f"need at least 3 (1 test + 1 val + 1 train)")
            pools[subject] = set(pool)

        common_stories = sorted(set.intersection(*(pools[s] for s in self.subjects)))
        if len(common_stories) < 2:
            raise ValueError(
                "per_sub requires at least two stories shared by every selected subject")

        random.Random(self.seed + fold * 1000).shuffle(common_stories)
        test_stories = common_stories[:1]
        val_stories = common_stories[1:2]
        held_out = set(test_stories + val_stories)

        result = {}
        for subject in self.subjects:
            train_stories = sorted(pools[subject] - held_out)
        ...
        train_union = {
            story for split in result.values() for story in split['train_stories']
        }
        val_union = {
            story for split in result.values() for story in split['val_stories']
        }
        test_union = {
            story for split in result.values() for story in split['test_stories']
        }
        if train_union & val_union or train_union & test_union or val_union & test_union:
            raise RuntimeError("Global story leakage detected in per_sub split")
...
def get_dataset_from_pairs(pairs, base_config, data_type='train'):
    ...
    """
    from speech_code.utils import DATASETS
    ...
    dataset_name = base_config.get('dataset_name', 'eeg_speech_v1')
