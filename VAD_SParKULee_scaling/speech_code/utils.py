...
def _allocate_sparkulee_prefix(items, recording_fraction):
    """Allocate a natural-order prefix while retaining baseline collection order."""
    allocation_items = sorted(items, key=lambda item: _natural_key(item[0]))
    target, allocations = prefix_allocation(
        [item[3] for item in allocation_items], recording_fraction
    )
    selected_by_path = {
        item[1]: keep_samples
        for item, keep_samples in zip(allocation_items, allocations)
    }
    allocation_plan = tuple(
        (*item, selected_by_path[item[1]]) for item in allocation_items
    )
    collection_plan = tuple(
        (*item, selected_by_path[item[1]]) for item in items
    )
    return target, allocation_plan, collection_plan
...
class EEG_Speech_v1(Dataset):
            ...
            for story_id in self.stories:
                ...
                try:
                    ...
                    if speech_labels is None:
                        continue
                    # 按滑动窗口收集样本
                    self._collect_samples(subject, story_id, speech_labels)
    ...
    def _load_and_preprocess_eeg(self, subject, story_id):
        ...
        if eeg.shape[0] == 57:
            ...
            if ch_transform is None:
                raise ImportError("ch_transform is required for EEG_Speech_v1 but was not found")
...
# ============================================================================
# SparKULee_VAD - SparKULee EEG + Silero VAD label Dataset
# ============================================================================
class SparKULee_VAD(Dataset):
    """
    SparKULee frame-level VAD Dataset.

    EEG is loaded from flat SparKULee preprocessed files:
        sub-001_audio-audiobook-1_LP-30_64Hz.npy

    Labels are loaded from Silero VAD TSV files:
        segments_audio-audiobook-1.tsv

    EEG arrays are shaped (64, T). Labels are generated at label_sfreq
    samples/sec, while EEG windows are sliced at sfreq samples/sec.
    """
    ...
    def __init__(
        ...
        exclude_subjects: list = None,
        stories: list = None,
        include_pairs: list = None,
        tmin: float = 0.0,
        tmax: float = 12.0,
        standardize: bool = True,
        data_type: str = 'train',
        sfreq: float = 64.0,
        label_sfreq: float = 250.0,
        stride=None,
        delay: int = 0,
        preproc: str = "LP-30",
        eeg_fs_tag: str | None = None,
        include_info: bool = False,
        ...
        **kwargs,
        ...
        self.data_type = data_type
        self.sfreq = sfreq
        self.raw_sfreq = raw_sfreq if raw_sfreq is not None else sfreq  # for on-the-fly resampling
        self.label_sfreq = label_sfreq
        self.points_per_sample = int((tmax - tmin) * self.sfreq)
        self.points_per_sample_label = int((tmax - tmin) * self.label_sfreq)
        self.stride = stride
        self.delay = delay
        self.preproc = preproc
        self.eeg_fs_tag = eeg_fs_tag or f"{int(sfreq)}Hz"
        self.include_info = include_info
        ...
        self.scaling_metadata = {
            ...
            "duration_sfreq": float(self.raw_sfreq),
            "subjects": {},
        }

        include_subjects = _normalize_sparkulee_subjects(include_subjects)
        exclude_subjects = set(_normalize_sparkulee_subjects(exclude_subjects) or [])
        requested_stories = set(stories) if stories is not None else None
        requested_pairs = None
        if include_pairs is not None:
            requested_pairs = {
                (_normalize_sparkulee_subjects([subject])[0], story)
                for subject, story in include_pairs
            }
        ...
        for path in sorted(os.listdir(data_path)):
            if not path.endswith(".npy"):
                continue
            parsed = self._parse_eeg_name(path)
            if parsed is None:
                continue
            subject, story, preproc, fs_tag = parsed
            if preproc != self.preproc or fs_tag != self.eeg_fs_tag:
                continue
            if include_subjects is not None and subject not in include_subjects:
                continue
            if subject in exclude_subjects:
                continue
            if requested_stories is not None and story not in requested_stories:
                continue
            if requested_pairs is not None and (subject, story) not in requested_pairs:
                continue
            eeg_files.append((subject, story, os.path.join(data_path, path)))

        if len(eeg_files) == 0:
            raise ValueError(
                "No SparKULee EEG files found. Check data_path, subjects, "
                "stories, preproc, and eeg_fs_tag."
            )

        files_by_subject = {}
        for subject, story, eeg_path in eeg_files:
            files_by_subject.setdefault(subject, []).append((story, eeg_path))

        for subject in sorted(files_by_subject):
            subject_items = []
            # The incoming order inherits baseline's sorted directory listing.
            for story, eeg_path in files_by_subject[subject]:
                arr = np.load(eeg_path, mmap_mode='r')
                time_dim = 0 if arr.shape[0] > arr.shape[1] else 1
                full_samples = int(arr.shape[time_dim])
                eeg_duration = full_samples / self.raw_sfreq
                labels = self._get_labels_for_story(story, eeg_duration)
                subject_items.append((story, eeg_path, labels, full_samples))

            target, allocation_plan, collection_plan = _allocate_sparkulee_prefix(
                subject_items, self.recording_fraction
            )
            unit_records = []
            for story, eeg_path, labels, full_samples, keep_samples in allocation_plan:
                unit_records.append(
                    {
                        "unit": story,
            ...
            for story, eeg_path, labels, full_samples, keep_samples in collection_plan:
                keep_labels = (
                    len(labels)
                    if keep_samples == full_samples
                    else int(keep_samples * self.label_sfreq / self.raw_sfreq)
                )
                if keep_labels > 0:
                    self._collect_samples(
                        subject, story, eeg_path, labels[:keep_labels]
                    )
            self.scaling_metadata["subjects"][subject.replace("sub-", "", 1)] = {
                "full_samples": sum(item[3] for item in subject_items),
                "selected_samples": target,
                "units": unit_records,
            }

        if len(self.samples) == 0:
            raise ValueError("No samples found for SparKULee. Check window length and labels.")

    @staticmethod
    def _parse_eeg_name(filename):
        ...
        preproc = parts[-2]
        fs_tag = parts[-1]
        if not subject.startswith("sub-") or not audio_key.startswith("audio-"):
            return None
        return subject, audio_key.replace("audio-", "", 1), preproc, fs_tag

    def _get_labels_for_story(self, story, eeg_duration):
        target_samples = int(eeg_duration * self.label_sfreq)
        cache_key = (story, target_samples)
        if cache_key in self.label_cache:
            return self.label_cache[cache_key]

        label_name = f"segments_audio-{story}.tsv"
        tsv_path = os.path.join(self.label_path, label_name)
        if not os.path.exists(tsv_path):
            raise FileNotFoundError(f"Label file not found: {tsv_path}")

        seg_df = pd.read_csv(tsv_path, sep='\t')
        labels = np.zeros(target_samples, dtype=np.int64)
        voice_df = seg_df[seg_df['label'] == 'voice']
        for _, row in voice_df.iterrows():
            start_sample = max(0, int(float(row['start']) * self.label_sfreq))
            end_sample = min(target_samples, int(float(row['end']) * self.label_sfreq))
    ...
    def _collect_samples(self, subject, story, eeg_path, speech_labels, stride=None):
        time_window_samples = self.points_per_sample_label
        if stride is None:
            if self.stride is not None:
                stride = int(self.stride * self.label_sfreq) if isinstance(self.stride, float) else self.stride
        ...
        for i in range(0, len(speech_labels), stride):
            ...
            if self.data_type == 'train':
                ...
                sample_labels = speech_labels[jittered_i:jittered_i + time_window_samples]
                onset_i = jittered_i
            self.samples.append((subject, story, eeg_path, onset_i / self.label_sfreq, sample_labels))

    def _load_eeg(self, eeg_path):
        if eeg_path not in self.open_files:
            self.open_files[eeg_path] = np.load(eeg_path, mmap_mode='r')
        return self.open_files[eeg_path]
    ...
    def __getitem__(self, idx):
        ...
        subject, story, eeg_path, onset, label = self.samples[idx]
        eeg_data = self._load_eeg(eeg_path)

        delay_samples = self.delay
        # Slice at raw_sfreq, then downsample to sfreq
        raw_points = int(self.points_per_sample * self.raw_sfreq / self.sfreq)
        raw_start = max(0, int((onset + self.tmin) * self.raw_sfreq) + int(delay_samples * self.raw_sfreq / self.sfreq))
        raw_end = raw_start + raw_points

        # Auto-detect orientation: larger dim is time
        if eeg_data.shape[0] > eeg_data.shape[1]:
            window = np.asarray(eeg_data[raw_start:raw_end, :], dtype=np.float32)  # (W, C)
            data = window.T  # (C, W)
        else:
            data = np.asarray(eeg_data[:, raw_start:raw_end], dtype=np.float32)  # (C, W)

        # Downsample if needed
        if self.raw_sfreq != self.sfreq:
            target_n = self.points_per_sample
            data = scipy_resample(data, target_n, axis=1)

        if data.shape[1] < self.points_per_sample:
            padded = np.zeros((data.shape[0], self.points_per_sample), dtype=np.float32)
            padded[:, :data.shape[1]] = data
            data = padded
        ...
        if self.include_info:
            # subject_id: 0-indexed integer for per-subject spatial attention
            subj_id = int(subject.replace("sub-", "")) - 1
            info = {
                "dataset": "sparkulee",
                "subject": subject,
                "subject_id": subj_id,
...
def _normalize_sparkulee_subjects(subjects):
    if subjects is None:
        return None
    normalized = []
    for subject in subjects:
        ...
        if subject.startswith("sub-"):
            ...
            normalized.append(f"sub-{int(subject):03d}")
    return normalized
