...
from scaling.subset import ALL_SUBJECTS, SubjectSplit
...
def _build_loader(config, partition, subject, stories):
    ...
    dataset_config["include_pairs"] = [[subject, story] for story in stories]
...
def evaluate_run(run_dir, config, subject_splits, device=None):
    ...
    for subject in ALL_SUBJECTS:
        ...
        val_loader = _build_loader(config, "val", subject, split.val_stories)
        test_loader = _build_loader(config, "test", subject, split.test_stories)
        ...
        metrics.update(
            {
                "subject": subject,
                "val_story": split.val_stories[0],
                "test_story": split.test_stories[0],
...
def _splits_from_config(config):
    serialized = config["general"]["scaling"]["subject_splits"]
    return {
        ...
        for subject, values in serialized.items()
    }
...
def main():
    ...
    summary = evaluate_run(run_dir, config, _splits_from_config(config))
