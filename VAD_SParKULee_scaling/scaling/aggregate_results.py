...
def collect_results(results_root):
    ...
    for scale_dir in sorted(results_root.glob("scale_*")):
        ...
        row = {
            ...
            "unique_selected_eeg_coverage_hours": manifest["unique_selected_eeg_coverage_hours"],
...
def write_outputs(rows, subject_values, output_dir):
    ...
    ax.plot(x, y, marker="o", linewidth=2, label="Mean across 23 subjects")
    ...
    ax.set_title("SParKULee Training-Duration Scaling")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("SParKULee Per-Subject Scaling Results")
...
def main():
    ...
    parser.add_argument(
        "--results-root",
        default=(
            "results/scaling/brain_magic_speech_v7/protocol_duration_prefix_train_only/protocol_shared_story/split_seed_84/"
            "subset_seed_1001/model_seed_1"
        ),
    )
    parser.add_argument(
        "--output-dir", default="results/scaling/summary_duration_prefix_train_only_shared_story_seed84_subset1001_model1"
    )
