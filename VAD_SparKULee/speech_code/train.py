    ...
    if dataset_key == "sparkulee_vad":
        ...
        for filename in os.listdir(data_path):
            ...
            if parts[-2] != preproc or parts[-1] != eeg_fs_tag:
                continue
            subj_raw = parts[0]  # e.g. "sub-001"
            subject = subj_raw.replace("sub-", "", 1)
            audio_key = "_".join(parts[1:-2])
            if audio_key.startswith("audio-"):
                story = audio_key.replace("audio-", "", 1)
                subjects.add(subject)
                stories.add(story)
                subj_stories_map.setdefault(subject, set()).add(story)
        # Keep only subjects with >= 3 stories (need 1 test + 1 val + >=1 train)
        subjects = sorted([s for s in subjects if len(subj_stories_map.get(s, set())) >= 3])
        return subjects, sorted(stories), {s: sorted(subj_stories_map[s]) for s in subjects}
...
def main(args):
    ...
    config["general"]["dataset_name"] = get_dataset_key(config, "train")
    ...
    # --- 嵌入 CLI 参数到 config（作为默认值，搜索空间可覆盖） ---
    ...
    # 根据 run_index 获取本次运行的超参数配置（覆盖 CLI 默认值）
    ...
    config_str = config_str.replace(' ', '').replace('[', '').replace(']', '').replace(',', '_')
...
def test_only(run_dir):
    ...
    if split_strategy == 'whole_data' and 'split_pairs' in config.get('data', {}):
        base_cfg = dict(get_dataset_config(config, "test"))
        base_cfg['dataset_name'] = get_dataset_key(config, "test")
...
def train(config, test_mode=False):
    ...
    elif split_strategy in ('single_subject', 'daily_stratified', 'daily_stratified_shared', 'cross_day', 'per_sub'):
    ...
    # 全量被试和试次集合
    all_subjects, all_stories, subj_stories_map = infer_split_items(config)
    dataset_key = get_dataset_key(config, "train")
    ...
    if test_mode:
        ...
        if dataset_key == "sparkulee_vad":
            if split_strategy == 'cross_subject':
                all_subjects = [f"{i:03d}" for i in range(1, 11)]
                all_stories = all_stories[:5]
            else:
                all_subjects = [f"{i:03d}" for i in range(1, 6)]
                all_stories = all_stories[:5]
        elif split_strategy == 'cross_subject':
            # 按被试划分 → 需要 ≥ 5 名被试
            ...
            all_stories = list(range(1, 11))                      # 10 个 story
        elif split_strategy in ('daily_stratified', 'daily_stratified_shared', 'per_sub'):
            # 每天分层随机/共享划分 → 每天需要 ≥ 5 个 story (3 test + 2 val)
            # per_sub → 只需要 ≥ 2 个 story (1 test + 1 val)
            all_subjects = [f"{i:02d}" for i in range(1, 3)]    # 2 名被试
            # 使用全部 50 个 story 以确保每天有足够的试次
    ...
    elif split_strategy == 'per_sub' and is_pretrain:
        # 预训练：收集所有被试的 (subject, story) pairs
        train_pairs = []
        val_pairs = []
        test_pairs = []
        for subject in all_subjects:
            subj_split = split[subject]
            for story in subj_split['train_stories']:
                train_pairs.append((subject, story))
            for story in subj_split['val_stories']:
                val_pairs.append((subject, story))
            for story in subj_split['test_stories']:
                test_pairs.append((subject, story))
        config['data']['split_pairs'] = {
            'train': train_pairs, 'val': val_pairs, 'test': test_pairs}
        config['data']['split_strategy'] = split_strategy
        print(f"[PRETRAIN per_sub] Built pairs from {len(all_subjects)} subjects")
    ...
    # 将划分结果写入 config
    if not (split_strategy in ('daily_stratified', 'daily_stratified_shared', 'per_sub') and is_pretrain):
    ...
    _data_split_strategy = config.get('data', {}).get('split_strategy',
                                                      split_strategy)
    if split_strategy == 'per_sub' and is_pretrain:
        # per_sub 预训练：从 (subject, story) pairs 构建各分区数据集
        base_cfg = dict(get_dataset_config(config, "train"))
        base_cfg['dataset_name'] = get_dataset_key(config, "train")
        train_dataset = get_dataset_from_pairs(
            config["data"]["split_pairs"]["train"], base_cfg, 'train')
        val_dataset = get_dataset_from_pairs(
            config["data"]["split_pairs"]["val"], base_cfg, 'val')
        test_dataset = get_dataset_from_pairs(
            config["data"]["split_pairs"]["test"], base_cfg, 'test')
        labels = [0, 1]
    elif split_strategy == 'whole_data':
        # whole_data 策略：按 (subject, story) pair 列表构造各分区数据集
        base_cfg = dict(get_dataset_config(config, "train"))
        base_cfg['dataset_name'] = get_dataset_key(config, "train")
        train_dataset = get_dataset_from_pairs(
            config["data"]["split_pairs"]["train"], base_cfg, 'train')
...
def _parse_subject_range(spec, width=2):
    """Parse subject range string into a set of zero-padded IDs."""
    result = set()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain-subjects", type=str, default=None,
                       help="预训练时仅使用指定被试（如 '1-15' 或 '1,5,10'）")
