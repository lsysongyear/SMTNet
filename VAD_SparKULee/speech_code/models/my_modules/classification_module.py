...
class ClassificationModule(LightningModule):
    ...
    def forward(self, x, subject_ids=None):
        ...
        Args:
            ...
            subject_ids: [batch] 0-indexed subject IDs for per-subject spatial attention
        """
        input_len = x.shape[-1]
    ...
    def _extract_ids(self, batch):
        ...
        if isinstance(info, dict) and 'subject_id' in info:
            ...
            if isinstance(subjects, torch.Tensor):
                return subjects.to(self.device)
            elif isinstance(subjects, list):
                return torch.tensor(subjects, device=self.device)
        if isinstance(info, (list, tuple)) and len(info) > 0 and isinstance(info[0], dict):
            if 'subject_id' in info[0]:
                ids = [d['subject_id'] for d in info]
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        x, y = batch[0], batch[1]
        subj = self._extract_ids(batch)
        y_hat = self(x, subject_ids=subj)
