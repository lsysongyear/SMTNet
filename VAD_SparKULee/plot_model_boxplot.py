"""
Plot per-subject macro_acc boxplot + scatter across all models (except vlaai) for SparKULee.
BrainMagic v7 placed on the far right.
"""
...
for model_dir in sorted(os.listdir(os.path.join(RESULTS_DIR, "speech-detection"))):
    ...
    split_dir = os.path.join(RESULTS_DIR, "speech-detection", model_dir, SPLIT)
...
for i, d in enumerate(data):
    ...
    ax.scatter(np.full_like(d, i + 1) + jitter, d, alpha=0.4, s=15,
...
ax.set_title("Per-Subject Macro Accuracy by Model (SparKULee, per_sub)")
